...
int main(void)
{
	...
	uart_send_string("Start Ra-01 receiver irq\r\n");
	...
	result = sx127x_set_op_mode(sx127x_conf.spi_conf, SX127X_OP_MODE_RX);
	if (result != SX127X_STATUS_OK)
	{
		uart_send_string("Error setting continous receive\r\n");
	}
	...
	while(1)
	{
		...
		if (new_data_flag)
		{
			result = sx127x_lora_read_data(sx127x_conf.spi_conf, &data[0], (uint8_t)MAX_DATA_SIZE, 2000);
			if (result != SX127X_STATUS_OK)
			{
